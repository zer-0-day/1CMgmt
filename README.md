...
Документация по `Install-Module`, `Import-Module` и исполнению скриптов: Microsoft Learn. citeturn0search0turn0search1turn0search2turn0search7
...
Подробнее об Execution Policy — в документации Microsoft (на Windows политика регулирует запуск скриптов, на не‑Windows с PowerShell 6+ — по‑другому). citeturn0search2turn0search7
...
`Install-Module` ставит модуль из PSGallery в одну из папок из `$env:PSModulePath`. citeturn0search0turn0search5
...
`Import-Module` загружает модуль в **текущую** сессию PowerShell. Для автозагрузки в новых сессиях добавьте эту команду в профиль, либо полагайтесь на **implicit import** при первом вызове функции. citeturn0search1turn0search6
...
Возврат кода `3010` трактуется как «успех, требуется перезагрузка», а у `robocopy` коды `0–7` считаются «успешными». citeturn0search21turn0search8turn0search3turn0search14
...
- **Execution Policy (Windows).** Если получаете ошибки вида «скрипты запрещены» — установите политику `RemoteSigned` для пользователя (см. выше). Групповая политика может переопределить локальные настройки. citeturn0search2turn0search12
- **Где лежит модуль.** Модули ставятся в папки из `$env:PSModulePath` — это влияет на поиск при `Import-Module`. citeturn0search5
- **Коды возврата.** Для MSI `3010` = успех + требуется перезагрузка; для Robocopy успешными считаются 0–7. Учитывайте это в своих скриптах/CI. citeturn0search21turn0search8
...
Выполните `Import-Module 1CMgmt` в текущей сессии. Имейте в виду: импорт действует только на текущую сессию; при открытии новой — импорт повторяется (или используйте implicit import/профиль). citeturn0search1turn0search6
...
Используйте `Install-1CPlatform -SetupPath '\\server\share\…'` — модуль скопирует дистрибутив в локальный кэш и поставит оттуда. Проверьте коды Robocopy (0–7 = success). citeturn0search8turn0search3

**MSI вернул код 3010. Это ошибка?**  
Нет. Это «успех, требуется перезагрузка», нормальное поведение многих MSI‑пакетов. citeturn0search21turn0search14
